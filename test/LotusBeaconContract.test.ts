...
describe("LotusBeaconContract", function () {
  ...
  it("should register a user for an event", async function () {
    await lotusBeaconContract.connect(addr1).registerForEvent(1);
    const userEventIndex = await lotusBeaconContract.getUserEventIndex(1, addr1.address);
  ...
  it("should greet another user", async function () {
    ...
    await lotusBeaconContract.connect(addr1).greet(1, userEventIndex, physicalSensingData);
    const greetData = await lotusBeaconContract.greetDataRecords(1, 0);
    expect(greetData.eventId).to.equal(1);
  ...
  it("should not greet the same user twice", async function () {
    ...
    await lotusBeaconContract.connect(addr1).greet(1, userEventIndex, physicalSensingData);
    await expect(
      lotusBeaconContract.connect(addr1).greet(1, userEventIndex, physicalSensingData)
